...
int log_index[N_SAMPLES];
float log_desired[N_SAMPLES];
float log_actual[N_SAMPLES];
...
//  // Test: motor back and forth at 50% for short bursts
...
//	      if (HAL_UART_Receive(&hcom_uart[COM1], &c, 1, 100) == HAL_OK) {
//	          if (c == 'a') {
//	              state = 1;
//	              while (state != 0) {}
//	          }
...
//void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
//if (htim == &htim2) {
//    // Safety check - always runs regardless of state
//    uint32_t adc = read_adc();
//    if (adc < 250 || adc > 3845) {
//        // Kill motor
//        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 2400);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim == &htim2) {
        ...
        if (state == 1) {
            ...
            static float desired = 100.0f;  // start at 100mA
            ...
            // Set PWM based on sign of output
            if (output >= 0) {
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 2400 - (int)output);
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 2400);
            } else {
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 2400);
                __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 2400 - (int)(-output));
            }

            // Log data
            log_index[counter] = counter;
            log_desired[counter] = desired;
            log_actual[counter] = actual;

            counter++;

            // Flip desired current every 100 cycles
            if (counter % 100 == 0) {
                desired = -desired;
            }

            // Done after 400 cycles
